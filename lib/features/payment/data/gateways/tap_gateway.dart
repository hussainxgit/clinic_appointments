...
class TapGateway implements PaymentGateway {
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'amount': amount,
          'currency':
              'KWD', // This should be dynamic based on the original payment
          'reason': 'requested_by_customer',
          'post': {'url': 'https://your-website.com/webhooks/refund'},
        }),
      );
  ...
  bool validateCallback(Map<String, dynamic> data) {
    ...
    ).convert(utf8.encode(stringToHash));
  ...
  // Helper method to map Tap status to our application's status
  PaymentStatusType _mapChargeStatus(String status) {
    switch (status) {
      case 'CAPTURED':
