...
class PatientNotifier extends _$PatientNotifier {
  ...
  PatientState build() {
    _loadPatients();
    return PatientState(patients: [], isLoading: true);
  }
  ...
  List<Patient> searchPatients(String query) {
    ...
    return state.patients.where((patient) =>
  ...
  Future<Result<Patient>> toggleStatus(String id) async {
    try {
      ...
      final newStatus = patient.status == PatientStatus.active
  ...
  Future<Result<Patient>> addAppointmentReference(String patientId, String appointmentId) async {
    try {
      final repository = ref.read(patientRepositoryProvider);
      ...
      final updatedAppointmentIds = List<String>.from(patient.appointmentIds)..add(appointmentId);
      final updatedPatient = patient.copyWith(appointmentIds: updatedAppointmentIds);
  ...
  Future<Result<Patient>> removeAppointmentReference(String patientId, String appointmentId) async {
    try {
      final repository = ref.read(patientRepositoryProvider);
      ...
      final updatedAppointmentIds = List<String>.from(patient.appointmentIds)..remove(appointmentId);
      final updatedPatient = patient.copyWith(appointmentIds: updatedAppointmentIds);
